...
open class SwiftMessages {
    ...
    private let messageQueue = DispatchQueue(label: "it.swiftkick.SwiftMessages", attributes: [])
    private var queue: [Presenter] = []
    private var delays = Delays()
    private var counts: [String: Int] = [:]
    private var _current: Presenter? {
        didSet {
            if oldValue != nil {
    ...
    private func dequeueNext() {
        guard _current == nil, queue.count > 0 else { return }
    ...
    private func internalHide(id: String) {
        if id == _current?.id {
            hideCurrent()
        }
        queue = queue.filter { $0.id != id }
    ...
    private weak var autohideToken: AnyObject?
    ...
    private func queueAutoHide() {
        ...
        if let pauseDuration = current.pauseDuration {
            ...
            messageQueue.asyncAfter(deadline: delayTime) { [weak self, weak current] in
...
extension SwiftMessages: PresenterDelegate {
    ...
    public func panStarted(animator _: Animator) {
        autohideToken = nil
    }

    public func panEnded(animator _: Animator) {
        queueAutoHide()
    }
...
extension SwiftMessages {
     ...
     */
    public class func viewFromNib<T: UIView>(_ filesOwner: AnyObject = NSNull()) throws -> T {
    ...
    /**
     ...
     - Returns: An instance of generic view type `T`.
     */
    public class func viewFromNib<T: UIView>(named name: String, filesOwner: AnyObject = NSNull()) throws -> T {
        let view: T = try internalViewFromNib(named: name, bundle: nil, filesOwner: filesOwner)
        return view
    }
    ...
    /**
     ...
     - Returns: An instance of generic view type `T`.
     */
    public class func viewFromNib<T: UIView>(named name: String, bundle: Bundle, filesOwner: AnyObject = NSNull()) throws -> T {
        let view: T = try internalViewFromNib(named: name, bundle: bundle, filesOwner: filesOwner)
        return view
    }

    fileprivate class func internalViewFromNib<T: UIView>(named name: String, bundle: Bundle? = nil, filesOwner: AnyObject = NSNull()) throws -> T {
